...
**Source priority for conflict resolution:** SPFL official → paid/free API → football-data.co.uk (for historical seasons) → Wikipedia (fallback only). The `match.source` column always records which source last wrote a given row, and `match_date`/score corrections from a higher-priority source overwrite lower-priority ones. Implemented in `src/processing/loader.py`'s `SOURCE_PRIORITY` table; a source not listed there is treated as lowest priority.

**Implementation status (Phase 2):** the SPFL official site's full league table and deeper archives are rendered by a third-party Opta widget (`secure.widget.cloud.opta.net`), not present in the server-rendered HTML a plain HTTP request receives - confirmed by fetching the live site while building `src/ingestion/spfl.py`. What *is* server-rendered, on both the league-wide and every per-club results page, is a small "last 3 results / next 3 fixtures" teaser (verified against `spfl.co.uk/league/premiership/results` and `spfl.co.uk/clubs/<slug>/results`). The fetcher works with this reality rather than around it: it fetches every current-season club's page (listed in `config/spfl_club_slugs.yaml`) and relies on natural upsert deduplication, since each match appears on two clubs' pages. This is sufficient for "current-season live results" at the documented daily ingestion cadence (§5) - a club would need 4+ unfetched matches to lose data - and the project doesn't need SPFL's rendered table anyway, since `src/analysis/standings.py` computes it directly from raw match data.
